     ...
     <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="md:grid-cols-2 gap-y-2 gap-x-4 grid space-y-2">
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>¿Qué tipo de elementos necesitas?</FormLabel>
              ...
              <FormControl>
                <div className="relative">
                  ...
                  <Eye className="absolute top-3 right-10 cursor-pointer" size={18} onClick={()=> setShowPassword(!showPassword)}/>
                    <Copy className="absolute top-3 right-2 cursor-pointer" onClick={()=> copyClipboard(field.value)} size={18} />
                </div>
